...
# Isolate resting data for per subject
for subject_id, subject_data in df.groupby('Individual'):
    
    resting_data = subject_data[subject_data['Phase'].isin(["phase1", "phase3"])]
    #resting_data = subject_data[subject_data['Phase'] == "phase1"]
...
for fold_idx, (train_index, test_index) in enumerate(logo.split(df_scaled, groups=groups)):
    ...
    train_rest = train_df[train_df['Phase'].isin(["phase1", "phase3"])][data_columns]
    #train_rest = train_df[train_df['Phase'] == "phase1"][data_columns]
...
for emotion in quastinare_columns:
    ...
    if len(clean_df) > 0:
        # Calculate Point-Biserial correlation (binary flag vs continuous/ordinal score)
